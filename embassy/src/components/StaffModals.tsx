...
export function StaffModals({
  ...
  return (
    <>
      <Modal open={modal === 'add'} title="Add Staff Member" onClose={onClose} className="modal--wide">
        <div className="staff-wizard">
          ...
          {addStep === 1 ? (
            <p className="staff-wizard__hint">
              Basic profile information for this staff account. You&rsquo;ll set section access in the
              next step.
            </p>
          ) : null}
          ...
          {addStep === 1 ? (
            ...
            >
              ...
              <label>
                ...
                <input
                  ...
                  placeholder="e.g., Visa Officer, Consul"
                  disabled={saving}
                />
              </label>
              <label>
              ...
              <label>
                ...
                <select
                  value={draft.status}
                  onChange={(e) => onDraftChange({ ...draft, status: e.target.value as StaffStatus })}
                  disabled={saving}
                >
            ...
            <div className="staff-access-body">
              ...
              <SectionAccessGrid sections={draftSections} onToggle={onToggleSection} disabled={saving} />
              <div className="modal-form__actions staff-form-actions">
                <button type="button" className="is-ghost" onClick={() => onSetAddStep(1)} disabled={saving}>
                  <ChevronLeft size={16} />
                  Back
                </button>
                <button type="button" className="is-primary" onClick={onCreateStaff} disabled={saving}>
                  <Check size={16} />
      ...
      <Modal open={modal === 'edit'} title="Edit Staff Member" onClose={onClose} className="modal--wide">
        {formError ? <div className="modal-form__error">{formError}</div> : null}
        <form
          className="modal-form staff-form-grid"
          onSubmit={(e) => {
      ...
      <Modal open={modal === 'access'} title="Manage Access" onClose={onClose} className="modal--wide">
        {formError ? <div className="modal-form__error">{formError}</div> : null}
        {activeMember ? (
          <div className="staff-access-body">
      ...
      <Modal open={modal === 'delete'} title="Delete Staff Member" onClose={onClose}>
        ...
        <p className="staff-delete-copy">
          Deactivate and remove <strong>{activeMember?.fullName}</strong> from the staff list? This
          action can be undone by adding them again later.
        </p>
