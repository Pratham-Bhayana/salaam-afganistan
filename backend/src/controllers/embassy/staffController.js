const { body } = require('express-validator');
const EmbassyStaff = require('../../models/EmbassyStaff');
const { EMBASSY_ROLES } = require('../../config/embassyPermissions');
const { ApiError, asyncHandler, success } = require('../../middleware/error');
const { parsePagination, escapeRegex } = require('../../utils/helpers');
const { activityFromReq } = require('../../services/embassyActivityService');

const createValidators = [
  body('firstName').trim().notEmpty(),
  body('lastName').trim().notEmpty(),
  body('email').isEmail(),
  body('password').isLength({ min: 8 }),
  body('role').isIn(Object.values(EMBASSY_ROLES)),
];

const list = asyncHandler(async (req, res) => {
  const { page, limit, skip } = parsePagination(req.query);
  const filter = { embassy: req.embassyId };
  if (req.query.role) filter.role = req.query.role;
  if (req.query.isActive !== undefined) filter.isActive = req.query.isActive === 'true';
  if (req.query.q) {
    const q = escapeRegex(req.query.q);
    filter.$or = [
      { email: new RegExp(q, 'i') },
      { firstName: new RegExp(q, 'i') },
      { lastName: new RegExp(q, 'i') },
    ];
  }

  const [data, total] = await Promise.all([
    EmbassyStaff.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit),
    EmbassyStaff.countDocuments(filter),
  ]);

  return success(res, data, { page, limit, total, pages: Math.ceil(total / limit) });
});

const getById = asyncHandler(async (req, res) => {
  const staff = await EmbassyStaff.findOne({ _id: req.params.id, embassy: req.embassyId });
  if (!staff) throw new ApiError(404, 'Embassy staff not found');
  return success(res, staff);
});

const create = asyncHandler(async (req, res) => {
  const exists = await EmbassyStaff.findOne({ email: req.body.email.toLowerCase() });
  if (exists) throw new ApiError(409, 'Email already in use');

  const passwordHash = await EmbassyStaff.hashPassword(req.body.password);
  const staff = await EmbassyStaff.create({
    embassy: req.embassyId,
    firstName: req.body.firstName,
    lastName: req.body.lastName,
    email: req.body.email.toLowerCase(),
    phone: req.body.phone,
    role: req.body.role,
    accessMode: req.body.accessMode || 'all',
    passwordHash,
    createdByEmbassyStaff: req.embassyStaff._id,
    isActive: req.body.isActive !== false,
  });

  await activityFromReq(req, {
    action: 'staff.create',
    resourceType: 'EmbassyStaff',
    resourceId: staff._id,
  });

  return success(res, staff, null, 201);
});

const update = asyncHandler(async (req, res) => {
  const staff = await EmbassyStaff.findOne({
    _id: req.params.id,
    embassy: req.embassyId,
  }).select('+passwordHash');
  if (!staff) throw new ApiError(404, 'Embassy staff not found');

  if (req.body.firstName != null) staff.firstName = req.body.firstName;
  if (req.body.lastName != null) staff.lastName = req.body.lastName;
  if (req.body.phone != null) staff.phone = req.body.phone;
  if (req.body.role != null) {
    if (!Object.values(EMBASSY_ROLES).includes(req.body.role)) {
      throw new ApiError(400, 'Invalid role');
    }
    staff.role = req.body.role;
  }
  if (req.body.accessMode != null) staff.accessMode = req.body.accessMode;
  if (req.body.isActive != null) staff.isActive = !!req.body.isActive;
  if (req.body.password) {
    staff.passwordHash = await EmbassyStaff.hashPassword(req.body.password);
  }

  await staff.save();
  await activityFromReq(req, {
    action: 'staff.update',
    resourceType: 'EmbassyStaff',
    resourceId: staff._id,
  });

  return success(res, staff);
});

const remove = asyncHandler(async (req, res) => {
  if (String(req.embassyStaff._id) === String(req.params.id)) {
    throw new ApiError(400, 'You cannot deactivate your own account');
  }
  const staff = await EmbassyStaff.findOneAndUpdate(
    { _id: req.params.id, embassy: req.embassyId },
    { $set: { isActive: false } },
    { new: true }
  );
  if (!staff) throw new ApiError(404, 'Embassy staff not found');

  await activityFromReq(req, {
    action: 'staff.deactivate',
    resourceType: 'EmbassyStaff',
    resourceId: staff._id,
  });

  return success(res, staff);
});

module.exports = {
  createValidators,
  list,
  getById,
  create,
  update,
  remove,
};
