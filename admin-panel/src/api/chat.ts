...
export async function ensureApplicationRoom(body: {
  embassyId: string;
...
export function embassyLabelFromRoom(room: ChatRoom) {
  const emb = typeof room.embassy === 'object' && room.embassy ? room.embassy : null;
  if (!emb) return 'Embassy';
  if (emb.code && emb.name) return `${emb.name} (${emb.code})`;
  return emb.name || emb.code || 'Embassy';
}
...
export function roomSubtitle(room: ChatRoom) {
  const emb = embassyLabelFromRoom(room);
  if (room.type === 'general') return `${emb} · Mission channel`;
  const app = typeof room.application === 'object' && room.application ? room.application : null;
  if (app?.status) return `${emb} · ${app.status.replaceAll('_', ' ')}`;
  return `${emb} · Case coordination`;
}
